...
| D-056 | bongfs root partition type GUID: `FFFFBF91-C780-49F6-9A07-E20A65A65B96`, generated once with a CSPRNG and hardcoded in `tools/mkimage/gpt.c`/`gpt.h` (`GPT_GUID_BONGFS_ROOT`) | deriving it from a name (UUIDv5); reusing an existing filesystem's type GUID | ARCHITECTURE §5.1 calls for "a custom type GUID recorded in DECISIONS.md" for the root partition; bongfs itself doesn't exist until M7.6, but `tools/mkimage` (M1.2) already needs to label an empty partition 3 with its eventual type so real GPT tools (and later, the kernel's own GPT scanner) can identify it |
