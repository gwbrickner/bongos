...
4. The GitHub Actions workflows (`ci`, `pr-policy`, `release`) and `tools/ci/install-deps.sh` already exist. Make the `format-check`, `host-tests`, `test`, `image`, `run`, `run-bios`, `debug`, and `gdb` targets work with them (`test` may be a no-op until M1.2 produces an image).
5. The boot harness already exists (`tests/harness/run-qemu.sh`, `run-matrix.sh`, `matrix.conf`). Wire `make test` to `run-matrix.sh` and `make test-full` to a `matrix-full.conf`. Don't rewrite the harness; extend it if needed.
...
### [x] M2.1 GDT, IDT, exceptions, hardening runtime `needs-owner`
**Needs:** M1.3
